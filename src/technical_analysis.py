...
def find_fair_value_gaps(
    ...
    for i in range(2, n):
        ...
        if c1["high"] < c3["low"]:
            ...
            low  = c1["high"]
            mid  = (high + low) / 2
            # Vérifier si comblé sur les bougies suivantes
...
def detect_mini_choch(
    ...
    if expected_direction == Direction.BULLISH:
        if not lows:
            return None
        # Dernier swing low (stop hunt potentiel)
        last_sl = lows[-1]
        # Vérifier que ce swing low est dans ou sous la zone de pullback
        if zone_low is not None and last_sl.price > zone_high:
            return None  # swing low trop haut, pas dans la zone
        # Chercher le swing high précédant ce swing low
        prior_highs = [sh for sh in highs if sh.index < last_sl.index]
        if not prior_highs:
            return None
        prior_sh = prior_highs[-1]
        # La bougie courante doit clôturer au-dessus du swing high précédent
        if last["close"] > prior_sh.price:
            # Volume check (non bloquant mais informatif)
            vol_ok = vol_r >= volume_multiplier
            logger.debug(
            ...
            return BougiConfirmation(
                ...
                entry_candle=last,
                entry_price=prior_sh.price,  # entrée au niveau du swing high cassé
                wick_ratio=0.0,
        ...
        if last["close"] < prior_sl.price:
            ...
            return BougiConfirmation(
                ...
                kind=ConfirmationType.MINI_CHOCH,
                entry_candle=last,
                entry_price=prior_sl.price,  # entrée au niveau du swing low cassé
                wick_ratio=0.0,
                body_ratio=abs(last["close"] - last["open"]) / (last["high"] - last["low"] or 1),
                volume_ratio=vol_r,
            )
...
def detect_false_breakout(
    ...
    for c in reversed(recent):
        ...
        if direction_of_breakout == Direction.BULLISH:
            # Mèche haute dépasse le niveau, close en dessous
            upper_wick = c["high"] - max(c["open"], c["close"])
...
def find_htf_targets(
    ...
    # [S5] find_fair_value_gaps filtre déjà les mitigés via find_fvg_nearest_price
